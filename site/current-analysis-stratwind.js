...
const GFS_RECENT_URL =
    "/analysis/polar_vortex/10hpa_60n_zonal_wind/gfs_recent.json";
...
// ============================================================
 // ERA5 + GFS RECENT EXTENSION
 // ============================================================

function mergeGfsRecentIntoBundle(
    bundle,
    gfsRecent
) {
    ...
    const era5LatestDate =
        gfsRecent.era5_latest_date
        ??
        null;


    gfsRecent.records.forEach(
        (record) => {
            ...
            if (
                ...
                !record.date
                ||
                record.value_ms === null
                ||
                record.value_ms === undefined
            ) {
...
async function fetchGfsRecent() {
    ...
    try {
        ...
        console.debug(
            "GFS recent stratwind unavailable:",
            error
        );
